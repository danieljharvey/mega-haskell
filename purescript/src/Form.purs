...
instance applyForm :: Semigroup e => Apply (Form internal e) where
  apply (Form f) (Form x) =
    Form \internal ->
      let
        { ui: uiF, result: resultF } = f internal
...
makeForm render validate = Form (\i -> { ui: render, result: V (validate i) })
...
getFormUI ::
  ...
  i ->
  UI
getFormUI (Form form) update i = (form i).ui update
...
getFormResult ::
  ...
  i ->
  Either e a
getFormResult (Form form) i = toEither $ (form i).result
...
validateString s = case NES.fromString s of
  Just a -> Right a
