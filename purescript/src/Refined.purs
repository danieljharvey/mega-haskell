...
instance showRefinedError :: (Show a) => Show (RefinedError a) where
  show (AndError (These a b)) = "And (" <> show a <> " && " <> show b <> ")"
  show (AndError (This a)) = show a
  show (AndError (That b)) = show b
  show (OrError a b) = "Or (" <> show a <> " && " <> show b <> ")"
  show (SizeEqualToError n a) = show a <> "'s length should equal " <> show n <> " but does not."
  show (SizeGreaterThanError n a) = show a <> "'s length should be greater than " <> show n <> " but is not."
  show (SizeLessThanError n a) = show a <> "'es length should be less than " <> show n <> " but is not."
  show (NotError) = "RefinedError with Not"
  show (LessThanError n a) = show a <> " should be less than " <> show n <> " but is not."
  show (GreaterThanError n a) = show a <> " should be greater than " <> show n <> " but is not."
  show (FromError n a) = show a <> " should be equal to or greater than " <> show n <> " but is not."
  show (ToError n a) = show a <> " should be equal to or less than " <> show n <> " but is not."
  show (FromToError m n a) = show a <> " should be equal to or greater than " <> show m <> " and equal to or less than " <> show n <> " but is not."
  show (EqualToError n a) = show a <> " should be equal to " <> show n <> " but is not."
  show (NotEqualToError n a) = show a <> " should not be equal to " <> show n <> " but it is."
...
instance predicateAnd ::
  ...
  validate _ x = case Tuple first second of
    ...
    where
    first = validate (undefined :: l) x

    second = validate (undefined :: r) x
...
instance predicateSizeEqualTo ::
  ...
  validate _ x = case length x == val of
    true -> Right x
...
instance predicateNot ::
  ...
  validate _ x = case validate (undefined :: a) x of
    Left _ -> Right x
    _ -> Left NotError
...
instance predicateLessThan ::
  ...
  validate _ x = case x < (fromInt val) of
    true -> Right x
...
instance predicateGreaterThan ::
  (Nat n, Ord x, Ring x) =>
  Predicate (GreaterThan n) x where
  validate _ x = case x > (fromInt val) of
    true -> Right x
...
instance predicateFromTo ::
  ...
  validate _ x = case (x >= fromInt lower) && (x <= fromInt upper) of
    true -> Right x
    false -> Left (FromToError lower upper x)
    where
    lower = toInt (undefined :: m)

    upper = toInt (undefined :: n)
