...
canIMoveTwo player = canMove (player.position <> player.direction)

turnAround :: Player -> Player
turnAround player = player { direction = invert player.direction }
...
incrementPlayerPosition moveAmount player@{ position: Coord pos, direction: Coord dir } = player { position = newPosition }
  where
  newPosition
...
incrementPlayerDirection amount player = player { position = newPosition }
  where
  newPosition :: Coord
  newPosition = player.position <> moveCoord

  moveCoord :: Coord
  moveCoord = createMoveCoord amount player.direction
