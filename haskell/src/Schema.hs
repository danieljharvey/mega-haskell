...
updateOlderToOldUser older =
  OldUser
    { oldFirstName = Name (Text.pack (olderFirstName older)),
...
instance
  ...
  Schema pristine earliest target
  where
...
instance
  ...
  WeakSchema pristine earliest target
  where
...
instance
  ...
  Migrate (this ': y ': xs) target pristine
  where
  migrate a =
...
instance
  ...
  TryMigrate (this ': y ': xs) target pristine
  where
  tryMigrate a =
...
instance
  ...
  TryMigrate '[this] target pristine
  where
...
instance
  ...
  GenerallyUpdate earliest target pristine
  where
...
instance
  ...
  GenerallyUpdate_ (x ': y ': xs) pristine
  where
  generallyUpdate_ =
    upgrade @pristine @y
      >>> generallyUpdate_ @(y ': xs) @pristine
...
instance
  ...
  MaybeUpdate earliest target pristine
  where
